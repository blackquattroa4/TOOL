...
class PurchaseApprovalRuleController extends Controller
{
	public function index(Request $request)
	{
		...
		if (!Session::has('alert-danger') && !Session::has('alert-warning') && !Session::has('errors')) {
			$rules = PurchaseApprovalRule::all()->toArray();
		...
		$suppliers = TaxableEntity::getSuppliers('name', 'asc');

		$employees = User::getAllStaff('name', 'asc');

		return view()->first(generateTemplateCandidates('rule.purchase_approval'),
						[
							'readonly' => false,
							'suppliers' => $suppliers,
							'employees' => $employees,
		...
		try {
			DB::transaction(function() use ($request) {
				foreach ($request->input('line') as $index => $id) {
					if ($id) {
						$rule = PurchaseApprovalRule::find($id);
