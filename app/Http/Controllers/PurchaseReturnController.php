...
class PurchaseReturnController extends Controller
{
	...
	public function createReturn($supplierId, Request $request)
	{
		$supplier = TaxableEntity::find($supplierId);
		if ($supplier->isNotActiveSupplier()) {
			return redirect(HistoryHelper::goBackPages(2))->with('alert-warning', str_replace("###", "", trans('vrm.Return ### can not be created')));
		}
	...
	public function createReturnPost(Request $request)
	{
		...
		try {
			DB::transaction(function() use ($request, &$title) {
				$header = PurchaseHeader::initialize('return', $request);
				$title = $header->title;
			});
		} catch (\Exception $e) {
		...
		return redirect(HistoryHelper::goBackPages(2))->with('alert-success', str_replace("###", "#".$title, trans('vrm.Return ### created.')));
	}
	...
	public function updateReturn($id, Request $request)
	{
		...
		if ($purchaseHeaderObj->isNotReturn() || $purchaseHeaderObj->isNotOpen()) {
			return redirect(HistoryHelper::goBackPages(2))->with('alert-warning', str_replace("###", "#".$purchaseHeaderObj->title, trans('vrm.Return ### can not be updated')));
		}
		$supplierId = $purchaseHeaderObj->entity_id;

		// load purchase return detail; errors imply redirect back, flashing input removes old value
		if (!Session::has('alert-danger') && !Session::has('alert-warning') && !Session::has('errors')) {
	...
	public function updateReturnPost($id, Request $request)
	{
		...
		// show meesage of success, and go back to 'dashboard'
		return redirect(HistoryHelper::goBackPages(2))->with('alert-success', str_replace("###", "#".$purchaseHeaderObj->title, trans('vrm.Return ### updated.')));
	}

	public function viewReturn($id, Request $request)
	{
		$purchaseHeaderObj = PurchaseHeader::find($id);
		if ($purchaseHeaderObj->isNotReturn()) {
			return redirect(HistoryHelper::goBackPages(2))->with('alert-warning', str_replace("###", "#".$purchaseHeaderObj->title, trans('vrm.Return ### can not be viewed')));
		}
		$supplierId = $purchaseHeaderObj->entity_id;
	...
	public function printReturn($id, Request $request)
	{
		$purchaseHeaderObj = PurchaseHeader::find($id);
		if ($purchaseHeaderObj->isNotReturn()) {
			return redirect(HistoryHelper::goBackPages(2))->with('alert-warning', str_replace("###", "#".$purchaseHeaderObj->title, trans('vrm.Return ### can not be printed')));
		}
	...
	public function approveReturn($id, Request $request)
	{
		$purchaseHeaderObj = PurchaseHeader::find($id);
		if ($purchaseHeaderObj->isNotReturn() || $purchaseHeaderObj->isNotOpen()) {
			return redirect(HistoryHelper::goBackPages(2))->with('alert-warning', str_replace("###", "#".$purchaseHeaderObj->title, trans('vrm.Return ### can not be approved')));
		}
		$supplierId = $purchaseHeaderObj->entity_id;
		...
		$optionArray['source'] = [
				//'supplier' => $supplierId,
				'title' => trans('vrm.Approve return'),
				//'document' => $purchaseHeaderObj['title'],
				//'status' =>  trans('status.'.ucfirst($purchaseHeaderObj['status'])),
				'currencyFormat' => $purchaseHeaderObj->currency->getFormat(true),
				'post_url' => '/' . $request->path(),
	...
	public function processReturn($id, Request $request)
	{
		$purchaseHeaderObj = PurchaseHeader::find($id);
		if ($purchaseHeaderObj->isNotReturn() || $purchaseHeaderObj->isNotOpen()) {
			return redirect(HistoryHelper::goBackPages(2))->with('alert-warning', str_replace("###", "#".$purchaseHeaderObj->title, trans('vrm.Return ### can not be processed')));
		}
		$supplierId = $purchaseHeaderObj->entity_id;
		...
		$optionArray['source'] = [
				//'supplier' => $supplierId,
				'title' => trans('vrm.Process return'),
				//'document' => $purchaseHeaderObj['title'],
				//'status' =>  trans('status.'.ucfirst($purchaseHeaderObj['status'])),
				'currencyFormat' => $purchaseHeaderObj->currency->getFormat(true),
				'post_url' => '/' . $request->path(),
	...
	public function processReturnPost($id, Request $request)
	{
		...
		// show meesage of success, and go back to 'dashboard'
		return redirect(HistoryHelper::goBackPages(2))->with('alert-success', str_replace("###", "#".$purchaseHeaderObj->title, trans('vrm.Return ### can not be processed')/*trans('vrm.Return ### processed.')*/));
	}

	public function releaseReturnPost($id, Request $request)
	{
		// all Ajax controller does not register with session-history
		// $this->removeFromHistory();

		$purchaseHeaderObj = PurchaseHeader::find($id);
		if ($purchaseHeaderObj->isNotReturn() || $purchaseHeaderObj->isNotApproved()) {
			return response()->json([ 'success' => false, 'errors' => [ 'general' => [ str_replace("###", "#".$purchaseHeaderObj->title, trans('vrm.Return ### can not be processed')) ]]]);
		}
