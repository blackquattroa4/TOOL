...
class WarehouseFormPdf extends Fpdf {
	...
	function __construct($input, $orientation='P', $unit='mm', $size='A4')
	{
		...
		foreach ($this->dataStruct['detail'] as $detail) {
			$hasLocation = strlen($detail['bins']);
			$this->Cell(50,12, $detail['sku'],$hasLocation?'T':'TB',0,'L',0);
			$lineX = $this->getX();
			$lineY = $this->getY();
			$this->Cell(120,12, "",$hasLocation?'T':'TB',0,'L',0);
			$this->setXY($lineX, $lineY);
			$this->MultiCell(120, 6, $detail['description'],0,'L');
			$this->setXY($lineX + 120, $lineY);
			$this->Cell(20,12, $detail['quantity'],$hasLocation?'T':'TB',0,'R',0);
			$this->ln(12);
			if ($hasLocation) {
				$this->Cell(50,6, "Bin # ",'B',0,'R',0);
				$this->Cell(120,6, $detail['bins'],'B',0,'L',0);
				$this->Cell(20,6, "",'B',0,'L',0);
				$this->ln(6);
			}
		}
	}
