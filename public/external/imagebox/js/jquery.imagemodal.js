...
(function($){
    ...
    $(objClicked).on('click', function(){
      ...
      switch (fileExtension) {
        ...
        case 'mp4':
          ...
          $('#img-preview').html('<video controls autoplay><source type="video/mp4" src="'+ _url +'" style="cursor: move; width: 100%; height: 95%;" /></video>');
          break;
        default:
      ...
      // kill content when closing; this is to prevent unintended keyboard/mouse event
      $(obj).find('#downloadableAttachmentModal').on('hidden.bs.modal', function(event) {
        $('#img-preview').html('');
      });
      $(obj).find('#downloadableAttachmentModal').modal('show');
    });
  ...
  */
  ...
  function initHtml(obj){
    var div = $('<div id="downloadableAttachmentModal" class="modal fade" tabindex="-1" data-backdrop="static" aria-hidden="false"></div>');
  ...
  */
  ...
  function switchImage(list_images){
    var $modal = $('#downloadableAttachmentModal');
    $('#downloadableAttachmentModal').on('click', '.switch', function(){
