...
<div class="modal fade" id="embeddedIncomeReceiptModal" tabindex="-1" role="dialog" aria-labelledby="embeddedIncomeReceiptModalLabel" data-backdrop="static" data-keyboard="false" aria-hidden="false">
  ...
  <div class="modal-dialog modal-lg" role="document">
			...
			<div style="height: 70vh; overflow-y: scroll;" class="modal-body">
      		...
      		<input type="hidden" name="_token" v-model="modal.csrf">
					...
					<div class="form-group">
						<label class="col-xs-2 col-sm-2 col-md-2 col-lg-2 control-label">{{ trans('forms.From') }}</label>
						<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
							<input id="code" type="code" name="code" class="form-control" v-model="form.code" disabled/>
						</div>
						<label class="col-xs-2 col-sm-2 col-md-2 col-lg-2 col-xs-offset-2 col-sm-offset-2 col-md-offset-2 col-lg-offset-2 control-label">{{ trans('forms.Pending credit') }}</label>
						<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
							<input name="amount_pending" class="form-control text-right" v-model="form.available" disabled/>
						</div>
					</div>
                ...
                <strong>@{{ errors['reference'][0] }}</strong>
						...
						<label class="col-xs-2 col-sm-2 col-md-2 control-label">{{ trans('forms.Amount received') }}</label>

						<div class="col-xs-2 col-sm-2 col-md-2" v-bind:class="{ 'has-error' : 'amount_received' in errors }">
							<input id="amount_received" name="amount_received" class="form-control text-right" v-model="form.amount_received" min="0.00" v-bind:step="modal.currency_min" type="number" onchange="updateUnappliedReceivableCredit()">
							<span class="help-block" v-if="'amount_received' in errors">
                <strong>@{{ errors['amount_received'][0] }}</strong>
              </span>
						</div>
					</div>
					...
					<table class="table table-striped">
						<tr>
							...
							<th class="col-xs-2 col-sm-2 col-md-2 text-right">{{ trans('forms.Balance') }}</th>
							<th class="col-xs-2 col-sm-2 col-md-2 text-right">{{ trans('forms.Apply') }}</th>
						</tr>
						<tr v-for="(id, index) in form.transactable_id">
							...
							<td class="text-right" v-bind:title="'{{ trans('finance.Total') }} ' + form.transactable_total[index]">
								<span v-if="form.transactable_credit[index]" class="badge badge-info">{{ trans('forms.Credit') }}</span>&emsp;@{{ form.transactable_balance[index] }}</td>
							<td>
                  ...
                  <input id="transactable[]" name="transactable[]" class="form-control text-right span1" v-bind:class="{ 'credit' : form.transactable_credit[index] }" v-model="form.transactable_applied[index]" min="0" v-bind:step="modal.currency_min" v-bind:max="form.transactable_balance_raw[index]" type="number" onchange="updateUnappliedReceivableCredit()">
        ...
        <div class="show-balance" style="position:fixed;bottom:3px;left:3px;text-decoration:none;color:#000000;background-color:rgba(255, 255, 255, 1.0);font-size:15px;padding:1em;display:none;z-index:10;">
          <p style="font-size:38px;font-weight:bold;line-height:38px;" >{{ trans('forms.Available') }} @{{ modal.available_display }}</p>
        </div>
			</div>

			<div v-if="!modal.readonly" class="modal-footer">
        <div class="col-md-12">
          <button v-for="(callback, display) in modal.action" type="button" class="btn btn-primary pull-right" style="margin-left:3px;" v-on:click="window[callback]()" v-bind:disabled="(form.transactable_applied.every(o => o == 0) && (form.amount_received == 0)) || (Object.keys(errors).length > 0)" v-html="display">
          </button>
        </div>
	    </div>
