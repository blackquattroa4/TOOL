...
<div class="modal fade" id="embeddedSalesQuoteModal" tabindex="-1" role="dialog" aria-labelledby="embeddedSalesQuoteLabel" data-backdrop="static" data-keyboard="false" aria-hidden="false">
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" onclick="$(this).closest('#historyModal').modal('hide')">
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
				...
				<table width="100%">
					<tr>
						<td>
							...
							<a v-if="modal.history && (modal.history.length > 0)" style="padding-right:20px;" href="#" data-toggle="modal" data-target="#embeddedSalesQuoteModal #historyModal"><span class="fa fa-2x fa-history"></span></a>
				...
				<form class="form-horizontal" role="form" method="POST" action="">
					...
					<div class="form-group">
						...
						<div class="col-md-4" v-bind:class="{ 'has-error' : 'customer' in errors }">
							<input id="customer" type="hidden" class="form-control" name="customer" v-model="form.entity" >
							<select id="_customer" class="form-control" name="_customer" v-model="form.entity" v-on:change="salesQuoteEntityChange()" v-bind:disabled="modal.readonly || (form.id != 0)">
								<option v-for="(customer, index) in modal.entity" v-bind:value="index">@{{ customer.display }}</option>
							</select>
					...
					<div v-for="(line, index) in form.line" class="form-group detail-line">
						...
						<div class="col-md-4" v-bind:class="{ 'has-error' : 'product.'+index in errors }">
							{{ trans('forms.Item') }}
							<select id="product[]" class="form-control" name="product[]" v-model="form.product[index]" v-on:change="fillSalesQuoteDefaultValue(index)" v-bind:disabled="modal.readonly">
						...
						<div class="col-md-2 col-md-offset-2" v-bind:class="{ 'has-error' : 'unitprice.'+index in errors }">
							...
							<input id="unitprice[]" type="number" style="text-align:right" v-bind:min="modal.currency[form.currency].min" v-bind:step="modal.currency[form.currency].min" class="form-control" name="unitprice[]" v-model="form.unitprice[index]" v-bind:readonly="modal.readonly"></input>
						...
						<div class="col-md-1" v-bind:class="{ 'has-error' : 'taxable.'+index in errors }">
							...
							<span v-if="'taxable.'+index in errors" class="help-block">
								<strong>@{{ error['taxable.'+index][0] }}</strong>
							</span>
						</div>
						-->
			...
			<div v-if="!modal.readonly || (Object.keys(modal.action).length > 0)" class="modal-footer">
				<div class="form-group">
					<div class="col-md-2">
						<button v-if="!modal.readonly" type="button" class="btn btn-primary pull-right" onclick="return addNewSalesQuoteLine();">+&nbsp;{{ trans('forms.Add line') }}</button>
					</div>
          <div class="col-md-10">
