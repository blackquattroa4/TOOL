					...
					<form class="form-horizontal" role="form" method="POST" action="{{ $post_url }}" >
						{{ csrf_field() }}
							...
							<div class="form-group">
								...
								<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
									<input name="amount_pending" class="form-control text-right" value="{{ $available }}" disabled/>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<label class="col-xs-2 col-sm-2 col-md-2 control-label">{{ $phrases['transact_term'] }}</label>
								<div class="col-xs-2 col-sm-2 col-md-2{{ $errors->has('amount_received') ? ' has-error' : '' }}">
									<input id="amount_received" name="amount_received" class="form-control text-right" value="{{ sprintf("%0.".$currencyFormat['fdigit']."f", old('amount_received')) }}" min="{{ sprintf("%0.".$currencyFormat['fdigit']."f", 0) }}" step="{{ $currencyFormat['min'] }}" type="number" />
									@if ($errors->has('amount_received'))
										<p class="help-block"><strong>{{ $errors->first('amount_received') }}</strong></p>
									@endif
								</div>
							</div>
						...
						<table class="table table-striped">
							<tr>
								...
								<th class="col-xs-2 col-sm-2 col-md-2">{{ trans('forms.Balance') }}</th>
								<th class="col-xs-2 col-sm-2 col-md-2">{{ trans('forms.Apply') }}</th>
							</tr>
						@foreach ($transactables as $oneTransactable)
							<tr>
								...
								@if ($oneTransactable['credit'])
									<span class="badge badge-info">{{ trans('forms.Credit') }}</span>&emsp;
								@endif
...
<div class='show-balance'>
	<p id='calculated_total'>{{ trans('forms.Available') }}<br>{{ $initialCredit }}</p>
</div>
  ...
  <script src="/js/ShowHideHint.js"></script>
	<script type="text/javascript">
		...
		$('input[type=number]').bind("change", function() {
			...
			$('#calculated_total').html('{{ trans("forms.Available") }}<br>' +
			...
			// warn if greater than available credit
			var pattern = /transactable\[[0-9]{1,}\]/g;
			if (pattern.test($(this).attr('name'))) {
				if (applying > total) {
					$(this).parent().addClass("has-error");
					if ($(this).parent().children().length == 1) {
						$(this).parent().append('<p class="help-block"><strong>greater than available</strong></p>');
					}
				$('[type="submit"]').attr('disabled', 'disabled');
					return false;
				}
