					...
					<table id="inboundbookingtable" class="table table-striped table-bordered" cellspacing="0" width="100%">
						<thead>
							<tr>
								...
								<th>{{ trans('forms.Reference') }}</th>
								<th>{{ trans('forms.Entity') }}</th>
								<th>{{ trans('forms.Estimate date') }}</th>
					...
					<table id="transfertable" class="table table-striped table-bordered" cellspacing="0" width="100%">
						<thead>
							<tr>
								...
								<th>{{ trans('forms.Reference') }}</th>
								<th>{{ trans('forms.Entity') }}</th>
								<th>{{ trans('forms.Delivery date') }}</th>
								<th>{{ trans('forms.Item count') }}</th>
								<th></th>
								<th></th>
							</tr>
		...
		@if ($controlSwitch['report-window'])
			<div id="reportwindow" class="panel panel-default">
				...
				@if ($controlSwitch['inventory-report'])
					<a href="{{ url('warehouse/inventory/print?date=' . \App\Helpers\DateHelper::dbToGuiDate(date("Y-m-d"))) }}" class="btn btn-sq-lg btn-info" ><span class="fa fa-2x fa-file-text-o"></span><br/>{!! str_replace(" ", "<br/>", trans("warehouse.Inventory report")) !!}</a>
				@endif
				@if ($controlSwitch['aging-report'])
					<a href="{{ url('warehouse/aging') }}" class="btn btn-sq-lg btn-info" ><span class="fa fa-2x fa-file-text-o"></span><br/>{!! str_replace(" ", "<br/>", trans("warehouse.Aging inventory")) !!}</a>
